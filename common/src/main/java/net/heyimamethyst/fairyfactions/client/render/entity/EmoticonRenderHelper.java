...
public class EmoticonRenderHelper
{
    ...
    public static void renderEmoticon(FairyEntity fairy, Font font, Quaternion cameraOrientation, PoseStack poseStack, MultiBufferSource pBuffer,
                                      double worldX, double worldY, double worldZ, float entityYRot, float partialTicks, int light)
    {
        ...
        if(fairy.isEmotional() && !fairy.isSleeping())
        {
            ...
            {
                renderIcon(new ItemStack(fairy.getWantedFoodItem()), poseStack, pBuffer, light);
    ...
    private static void renderIcon(ItemStack icon, PoseStack poseStack, MultiBufferSource buffers, int combinedLightIn)
    {
        if (!icon.isEmpty())
        {
    ...
    private static void renderText(FairyEntity fairy, ItemStack icon, Font font, Quaternion cameraOrientation, PoseStack poseStack, MultiBufferSource buffers, int combinedLightIn)
    {
        ...
        font.drawInBatch(mutableComponent, width, 0f, -1, false, matrix4f, buffers, true, 0, combinedLightIn);
